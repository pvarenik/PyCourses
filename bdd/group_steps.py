...
@pytest.allure.step("a group list")
@given("a group list")
def group_list(db):
    return db.get_group_list()

@pytest.allure.step("a group with name={name}, header={header} and footer={footer}")
@given("a group with <name>, <header> and <footer>")
def new_group(name, header, footer):
    return Group(name=name, header=header, footer=footer)

@when("I add the new group to the list")
def add_new_group(app, new_group):
    with pytest.allure.step("When I add the new group to the list"):
...
@pytest.allure.step("a non-empty group list")
...
@pytest.allure.step("a random group from the list")
